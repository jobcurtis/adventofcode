...
object Day02 : Year2023() {
  ...
  override fun part1() = games.filter { it.isPossible(bag) }.sumOf { it.id }
  ...
  private fun Game.isPossible(bag: BlockCollection) =
    rounds.all { it.all { (colour, count) -> bag.getOrDefault(colour, 0) >= count } }
