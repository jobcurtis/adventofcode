...
class IntCodeComputer(internal val memory: IntArray, internal val input: IntArray = intArrayOf()) {
    ...
    private fun interpret(instr: Instr): Unit = when (instr) {
        is Instr.Add -> memory[instr.out] = instr[1] + instr[2]
        is Instr.Mul -> memory[instr.out] = instr[1] * instr[2]
        is Instr.Inp -> memory[instr.out] = input.getOrNull(inputPosition++) ?: readln().toInt()
        is Instr.Out -> output += memory[instr.out]
